...
def main():
    ...
    parser.add_argument("--test_only", action = 'store_true', 
                         help="Prepare test data only")
    args = parser.parse_args()

    handler = DataHandler(args.data_dir, args.task_name)
    if args.test_only:
        if args.task_name == 'nips':
            handler.write_test('2015_papers', 'test_TopoSort.tsv', args.out_dir)
            handler.write_test('2015_papers', 'test_TSP.tsv', args.out_dir)
        elif args.task_name == 'sind':
            handler.write_test_sind('test', 'test_TopoSort.tsv', args.out_dir)
            handler.write_test_sind('test', 'test_TSP.tsv', args.out_dir)
        else:
            handler.write_test('test', 'test_TopoSort.tsv', args.out_dir)
            handler.write_test('test', 'test_TSP.tsv', args.out_dir)
